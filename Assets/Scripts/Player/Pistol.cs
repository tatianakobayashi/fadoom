...
public class Pistol : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        if (isShot == true && ammoClipLeft > 0 && isReloading == false)
        {
            ...
            if (Physics.Raycast(ray, out hit, pistolRange, layerMask))
            {
                ...
                //hit.collider.gameObject.SendMessage("pistolHit", 0, SendMessageOptions.DontRequireReceiver);
                

                if (hit.transform.CompareTag("Enemy"))
                {
                    hit.collider.gameObject.GetComponent<Enemy>().PistolHit(pistolDamage);
                ...
                else if (hit.transform.CompareTag("Pagligod"))
                {
                    hit.collider.gameObject.GetComponent<PagliGodController>().PistolHit(pistolDamage);
                    Instantiate(bloodSplat, hit.point, Quaternion.identity);
                    hit.collider.gameObject.SendMessage("HiddenShot", transform.parent.transform.position, SendMessageOptions.DontRequireReceiver);
                }
    ...
    public void MoreAmmo(int ammo)
    {
        ...
        string message = "+ " + ammo.ToString() + " ammo";
